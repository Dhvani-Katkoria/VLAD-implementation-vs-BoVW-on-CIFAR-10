...
des_list = np.empty(shape=[0,64])#descriptor length of surf
image_des_len = []
features = []
labels=[]
...
with open('CIFAR_SURF_DESC.csv', 'w', newline='') as file:
    writer = csv.writer(file, delimiter=',')
    writer.writerows(des_list)

with open('CIFAR_SURF_Label_Len.csv', 'w', newline='') as file:
    writer = csv.writer(file, delimiter=',')
